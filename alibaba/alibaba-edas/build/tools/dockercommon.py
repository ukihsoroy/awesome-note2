...
def getEdasConf():
    conffile = os.path.join(BASEDIR, "build", "dockerconfig.json")
    with open(conffile, 'r') as fp:
        content = fp.read()
    ...
    with open(conffile, 'r') as fp:
        ...
        if "apps" in confs:
            ...
            for conf in confs['apps']:
                ...
                userId = conf['userId']
                target = conf['target']
                deployType = conf['type']
                deployToStr = conf['deployToStr']
                packageVersion = conf['packageVersion']
                description = conf['description']
                imageUrl = conf['imageUrl']
                regionId = conf['regionId']
                batch = conf['batch']
                app = AppConfig(appName, appId, userId, target,deployType,deployToStr,packageVersion,description,imageUrl,regionId,batch)
                apps.append(app)
        if len(apps) > 0:
            return apps
    return None
...
class AppConfig():
    def __init__(self, appName, appId, userId, target,deployType,deployToStr,packageVersion,description,imageUrl,regionId,batch):
        self.appName = appName
        self.appId = appId
        self.userId = userId
        self.target = target
        self.deployToStr = deployToStr
